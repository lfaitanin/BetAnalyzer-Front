...
export default function BolasDe3Page() {
  ...
  const loadBets = async (filters: BetHistoryFilters) => {
    try {
      setLoading(true);
      const history = await getBetHistory({ ...filters, category: 'ThreePointers' });
      setBets(history);
      setError(null);
    } catch (err) {
  ...
  return (
    <div className="space-y-4 md:space-y-6 p-2 md:p-6 bg-gray-50">
      <h1 className="text-xl md:text-2xl font-bold text-gray-900">Histórico de Apostas - Bolas de 3</h1>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {bets.map((bet) => (
                <tr key={bet.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 md:px-6 py-2 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-900">R$ {bet.stake.toFixed(2)}</td>
                  ...
                  <td className="px-3 md:px-6 py-2 md:py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      bet.status === 'Ganhou' ? 'bg-green-100 text-green-800' :
                      bet.status === 'Perdeu' ? 'bg-red-100 text-red-800' :
